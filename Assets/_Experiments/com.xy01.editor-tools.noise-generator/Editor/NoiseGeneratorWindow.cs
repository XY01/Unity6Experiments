...
// - Add layering options. Noise type, freq, blend mode (add,mul,div...), range
    public class NoiseGeneratorWindow : EditorWindow
    {
        ...
        private string _filePathRelativeAssets = "/_Project/Noise generator/Textures/";
        ...
        private void OnGUI()
        {
            ...
            _noiseType =
                (Noise)EditorGUILayout.Popup("Noise Type:", (int)_noiseType, System.Enum.GetNames(typeof(Noise)));
            ...
            _filePathRelativeAssets = EditorGUILayout.TextField("File Path", _filePathRelativeAssets);
            _computeShader =
                EditorGUILayout.ObjectField("Compute Shader", _computeShader, typeof(ComputeShader), false) as
                    ComputeShader;
            ...
            if (GUILayout.Button("Save"))
                SaveTextureAsPNG(_renderTexture, _filePathRelativeAssets, _fileName);
            ...
            if (_renderTexture != null)
            {
                ...
                EditorGUI.DrawPreviewTexture(rect, _renderTexture, null, ScaleMode.ScaleToFit, 1, 0,
                    ColorWriteMask.Red);
            }
        }
        ...
        private void GenerateNoise()
        {
            ...
            int kernelHandle =
                _computeShader.FindKernel(_volumeTexture ? _noiseType.ToString() + "3D" : _noiseType.ToString());
            ...
            if (_volumeTexture)
            {
                ...
                _computeShader.Dispatch(minMaxKernel, _renderTexture.width, _renderTexture.height,
                    _renderTexture.volumeDepth / 8);
                ...
                _computeShader.Dispatch(remapKernel, _renderTexture.width / 8, _renderTexture.height / 8,
                    _renderTexture.volumeDepth / 8);
            ...
            if (_volumeTexture)
            {
                SaveRenderTextures.Save3D(
                    _renderTexture,
                    _filePathRelativeAssets + _fileName + " Volume",
        ...
        private void SaveTextureAsPNG(RenderTexture renderTexture, string filePathRelativeAssets, string fileName)
        {
            ...
            File.WriteAllBytes(Application.dataPath + filePathRelativeAssets + fileName + ".png", bytes);
        }
    }
}
#endif
