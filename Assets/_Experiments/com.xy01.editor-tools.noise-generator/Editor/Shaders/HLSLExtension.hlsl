...
float noise(float2 p)
{
    ...
    float2 b = a - o + K2;
    float2 c = a - float2(1.0, 1.0) + 2.0 * K2;
...
// ------------------------ SHARD NOISE --------------------------------
// https://www.shadertoy.com/view/mlKfzd
//
#define tau 6.283185307179586
float4 hash(float4 p)
{
    p = float4(dot(p, float4(127.1, 311.7, 74.7, 24.6)), 
        dot(p, float4(269.5, 183.3, 246.1, 123.5)), 
        dot(p, float4(113.5, 271.9, 124.6, 223.8)),
        dot(p, float4(222.3, 145.2, 211.3, 239.5)));
    
    p = frac(sin(p) * 43758.5453123);
    return p;
}

float shard_noise(in float4 p, in float sharpness)
{
    float4 ip = floor(p);
    float4 fp = frac(p);
    ...
    for (int w = -1; w <= 1; w++) {
        for (int z = -1; z <= 1; z++) {
            for (int y = -1; y <= 1; y++) {
                for (int x = -1; x <= 1; x++) {
                    float4 o = float4(x, y, z, w);
                    float4 io = ip + o;
                    float4 h = hash(io);
                    float4 r = fp - (o + h);

                    float W = exp2(-tau*dot(r, r));
                    float s = sharpness * dot(r, hash(io + float4(11, 31, 47, 59)) - 0.5);
                    v += W * s*rsqrt(1.0+s*s);
                    t += W;
                }
            }
